...
function normalizeIndexSql(sql) {
  if (!sql) return '';
  return String(sql)
    // Index names are often auto-generated by PocketBase and include the collection id.
    // We normalize them away so bootstrap remains idempotent across fresh installs and
    // migration-driven installs.
    .replace(/CREATE\s+UNIQUE\s+INDEX\s+`[^`]+`/i, 'CREATE UNIQUE INDEX `__name__`')
    .replace(/CREATE\s+INDEX\s+`[^`]+`/i, 'CREATE INDEX `__name__`')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function ensureCollection(token, def) {
  ...
  if (found) {
    ...
    const existingIndexes = found.indexes || [];
    const existingNormalized = new Set(existingIndexes.map(normalizeIndexSql));
    const missingIndexes = desiredIndexes.filter((idx) => !existingNormalized.has(normalizeIndexSql(idx)));
      ...
      try {
        patched = await pbFetch(`/api/collections/${found.id}`, {
          ...
          body: {
            ...
            ...(missingIndexes.length ? { indexes: [...existingIndexes, ...missingIndexes] } : {}),
          },
        });
      } catch (err) {
        // PocketBase rejects duplicate index *definitions* (even if names differ). If we
        // misdetected an index as missing, don't fail bootstrap for an already-correct DB.
        const detail = err?.detail;
        const duplicate = JSON.stringify(detail || {}).includes('validation_duplicated_index_definition');
        if (!duplicate) throw err;
        console.warn('[pb_bootstrap] ignoring duplicate index definition on', def.name);
        patched = found;
      }
