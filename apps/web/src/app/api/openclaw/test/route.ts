...
export async function POST(req: NextRequest) {
  ...
  try {
    invokeRes = await fetchWithTimeout(new URL('/tools/invoke', base), {
      ...
      headers: {
        ...
        'x-mission-control': '1',
        'x-mission-control-source': 'openclaw-test',
  ...
  if (!invokeRes.ok) {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: invokeRes.status === 401 ? 'Unauthorized token. Copy the Tools Invoke token from OpenClaw → Overview.' : msg,
      },
      { status: 502 }
    );
  }
  ...
  try {
    sendProbeRes = await fetchWithTimeout(new URL('/tools/invoke', base), {
      ...
      body: JSON.stringify({
        tool: 'sessions_send',
        commandId: `${sendReqId}-delivery-probe`,
