...
export async function POST(req: NextRequest) {
  ...
  if (!sessionKey) {
    return NextResponse.json({ ok: false, error: 'sessionKey required' }, { status: 400 });
  }

  const params = JSON.stringify({ key: sessionKey });
  const res = await runOpenClaw(['gateway', 'call', 'sessions.delete', '--params', params, '--json', '--timeout', '10000'], {
    timeoutMs: 12_000,
  });
  if (!res.ok) {
    const detail = redactText([res.message, res.stderr, res.stdout].filter(Boolean).join('\n')).trim();
    return NextResponse.json({ ok: false, error: detail || 'Failed to delete session.' }, { status: 502 });
  }
