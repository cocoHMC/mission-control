...
export async function GET(req: NextRequest) {
  ...
  const res = await runOpenClaw(['nodes', 'pending', '--json'], { timeoutMs: 12_000 });
  if (!res.ok) {
    const detail = redactText([res.message, res.stderr, res.stdout].filter(Boolean).join('\n')).trim();
    return NextResponse.json({ ok: false, error: detail || 'Failed to load pending nodes.' }, { status: 502 });
  }

  const stdout = String(res.stdout || '').trim();
  try {
    const parsed = stdout ? JSON.parse(stdout) : [];
    return NextResponse.json({ ok: true, pending: parsed });
  } catch {
