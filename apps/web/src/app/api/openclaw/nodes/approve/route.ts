...
export async function POST(req: NextRequest) {
  ...
  const res = await runOpenClaw(['nodes', 'approve', requestId, '--json'], { timeoutMs: 20_000 });
  if (!res.ok) {
    const detail = redactText([res.message, res.stderr, res.stdout].filter(Boolean).join('\n')).trim();
    return NextResponse.json({ ok: false, error: detail || 'Failed to approve node.' }, { status: 502 });
  }
