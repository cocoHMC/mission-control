...
export function WorkspacesClient({ initialWorkspaces, initialProjects }: Props) {
  ...
  async function deleteWorkspace(id: string) {
    if (!window.confirm('Delete this workspace? Projects are detached, not deleted.')) return;
  ...
  return (
    <div className="grid min-h-0 gap-4 lg:grid-cols-[380px_1fr]">
      <form onSubmit={createWorkspace} className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <div className="mt-2 text-xs text-muted">Use workspaces to separate business units, product lines, or deployment environments.</div>
        ...
        <div className="mt-3 space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              className="mt-2"
              placeholder="What this workspace owns, and how teams should run it."
            />
          </div>
        </div>
          ...
          {workspaces.map((workspace) => {
            ...
            return (
              <div key={workspace.id} className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
                ...
                >
                  <Input name="name" defaultValue={workspace.name || ''} placeholder="Workspace name" className="h-9" />
                ...
                {linked.length ? (
                  <div className="mt-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted">Projects in workspace</div>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
